...
Cover the object with a grid of boxes of side length $\varepsilon$. We count $N(\varepsilon)$, the number of boxes that intersect the object. We know that $N(\varepsilon)$ grows as a power law of $\varepsilon$, so we write $N(\varepsilon) \approx C(\varepsilon)^{-s}$ where $s$ is the box-counting dimension and $C$ is some constant. Taking logarithms of both sides creates an equation $\log N(\varepsilon) \approx s \left( \frac{1}{\varepsilon} \right) + \log C$.
